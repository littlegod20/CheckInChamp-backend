...
import { updatePollResults } from "../slack_activities/pollUpdates";


// ✅ Function to Open the Poll Modal
export const openCreatePollModal = async (triggerId: string, optionCount: number = 2) => {
                ...
                blocks: [
...

                    {
                        ...
                        element: {
                            ...
                            action_id: "poll_type",
                            options: [
                                { text: { type: "plain_text", text: "Select One" }, value: "single" },
                                { text: { type: "plain_text", text: "Select Many" }, value: "multiple" },
                                { text: { type: "plain_text", text: "Rate 1-5" }, value: "scale" },
                            ],
                        },
                    ...
                    // Anonymous Voting (Optional)
                    {
                        ...
                        text: { type: "mrkdwn", text: "*Would you like to make the poll anonymous?*" },
                        accessory: {
                            type: "checkboxes",
                            action_id: "anonymous",
                            options: [{ text: { type: "plain_text", text: "Make anonymous" }, value: "yes" }],
                        },
                    },
                    ...
                    // Select Channel to Post Poll
                    {
                        ...
                        label: { type: "plain_text", text: "Select a Channel to Post the Poll" },
                    },
                ],
...
slackApp.view("create_poll", async ({ ack, body, view }) => {
    ...
    try {
        const question = view.state.values.question_block.question.value ?? "default_value";
        ...
        const type = view.state.values.type_block.poll_type?.selected_option?.value ?? "default_value";
        const anonymous = (view.state.values.anonymous_block.anonymous?.selected_options?.length ?? 0) > 0;
        const channelId = view.state.values.channel_block.select_channel.selected_conversation;
            ...
            blocks: [
                ...
                ...(type === "scale"
                    ? [
                          {
                              type: "section",
                              text: { type: "mrkdwn", text: "*Rate this on a scale of 1-5:*" },
                              accessory: {
                                  type: "static_select",
...
slackApp.action("vote_single", async ({ ack, body, action }) => {
    ...
    try {
        ...
        await poll.save();
        await updatePollResults(body.channel?.id!, body.message?.ts!, poll);
        ...
        await slackApp.client.chat.postEphemeral({
            channel: body.channel?.id || "",
            user: userId,
            text: `✅ Your vote for *${poll.options[parseInt(selectedIndex)]}* has been recorded.`,
        });
    } catch (error) {
